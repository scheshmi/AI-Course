...
class Sudoku():
    def __init__(self, sudoku: np.array, fixed_entries: np.array = None) -> None:
    ...
    def random_filling_zeros(self) -> None:
        for num in range(9):
            block_indices = self.get_block_indices(num)
            block = self.sudoku[block_indices]
            zero_indices = [idx for i, idx in enumerate(
    ...
    def get_block_indices(self, n: int, ignore_fixed: bool = False) -> list:
        row = (n // 3) * 3
        col = (n % 3) * 3
    ...
    def get_column_indices(self, i: int) -> list:
        column = i
    ...
    def make_candidate_sudoku(self) -> np.array:
        ...
        square_1, square_2 = [self.get_block_indices(block, ignore_fixed=True)[
            idx] for idx in random_squares]
        new_sudoku[square_1], new_sudoku[square_2] = new_sudoku[square_2], new_sudoku[square_1]
        return new_sudoku


def display(grid: np.array) -> None:
